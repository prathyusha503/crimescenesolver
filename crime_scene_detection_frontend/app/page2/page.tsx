...
//       localStorage.setItem("latestUploadedVideo", file.name); // ADD THIS
//     } catch (err) {
//       console.error("Upload failed", err);
//     }
...
//     router.push("/page3"); // UPDATED NAVIGATION HANDLER
//   };
...
//         className="fixed bottom-30 right-140 bg-gray-500 text-white px-4 py-2 rounded"
//       >
//         Next
//       </button>
//     </div>
...
export default function Page2() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      localStorage.setItem("latestUploadedVideo", file.name);
    } catch (err) {
      console.error("❌ Upload failed", err);
  ...
  useEffect(() => {
    ...
    const extractAndDisplayFrames = async () => {
      ...
      for (let i = 0; i < totalFramesToExtract; i++) {
        ...
        const frameData = canvas.toDataURL("image/jpeg");
        newFrames.push(frameData);
        setFrames([...newFrames]); // Triggers frame updates
        setCurrentFrame(newFrames.length - 1);
      }
  ...
  return (
    <div className="flex items-center justify-center h-screen w-full bg-black relative">
      {/* Detective GIF */}
      <div className="fixed left-[20rem] bottom-[15rem] z-0 h-125">
        <img src="/videos/middle.gif" alt="Detective walking" />
      </div>
      ...
      >
        <motion.div
          ...
          transition={{ type: "spring", stiffness: 100, damping: 16 }}
          className="overflow-hidden shadow-lg border border-gray-700 relative flex items-center justify-center cursor-pointer"
        >
          {!showBox ? (
            <div className="flex items-center justify-center w-full h-full">
              <Image
      ...
      <button
        onClick={handleNextClick}
        className="fixed bottom-25 right-20 bg-gray-600 text-white px-10 py-4 rounded-xl hover:bg-gray-600 transition"
      >
        Next
      </button>
    </div>
